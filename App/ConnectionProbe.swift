...
/// tailnet" and "the server is down" arrive as the same spinner-then-nothing
/// unless the probe names them apart.
///
/// `reachableButUnauthorized` is deliberately a SUCCESS for reachability: a 403
/// is the API answering, which proves the whole network path works and narrows
/// the problem to the secret. Reporting it as a flat failure would send someone
/// hunting a network fault that the probe just disproved.
enum ProbeOutcome: Equatable {
    case working(eventCount: Int)
    ...
    /// Whether the server was proved to be there. A 403 counts — it is the
    /// server talking.
    ...
    /// Whether the app can actually load the feed with this configuration.
    ...
    var message: String {
        ...
        case .working(let count):
            return "Connected and authorized — the server returned \(count) event\(count == 1 ? "" : "s")."
        case .reachableButUnauthorized:
            return "Server reached, but the secret is missing or wrong (403). "
...
///
/// Two requests, because one cannot answer both questions. `/health` is the
/// only unauthenticated route on the API, so it separates "the server is there"
/// from "the secret is wrong"; the events route then proves the credential
/// actually works and says how much data came back. A probe that only called
/// the authenticated route would report an identical failure for a down server
/// and a bad secret.
struct ConnectionProbe {
    ...
    func run() async -> ProbeOutcome {
        ...
        do {
            ...
            // answered is not a healthy Barry API. Say that here rather than
            // letting the authenticated call below blame the secret for it.
            if let http = response as? HTTPURLResponse, !(200..<300).contains(http.statusCode) {
                return .serverError(status: http.statusCode, detail: "unexpected reply from \(ServerConfig.healthPath)")
            }
        } catch {
        ...
        do {
            let page = try await EventsClient(config: config, urlSession: urlSession).events(limit: 1)
            return .working(eventCount: page.events.count)
    ...
    static func classify(apiError error: EventsError) -> ProbeOutcome {
        switch error {
        case .badURL:
            return .badURL
