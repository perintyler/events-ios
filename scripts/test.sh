#!/usr/bin/env bash
# Full verification loop for the Events iPhone app: regenerate the project, then
...
if [ -z "$SIM_ID" ]; then
  ...
  echo "  Set EVENTS_IOS_SIM, or see: xcrun simctl list devices available" >&2
  exit 1
fi
...
xcodebuild -project "${SCHEME}.xcodeproj" -scheme "${SCHEME}" \
  ...
  test 2>&1 | tee /tmp/events-ios-test.log \
...
echo ""
if [ "$STATUS" -eq 0 ]; then
  echo "All tests passed. (Check the log for skips: grep -c 'was skipped' /tmp/events-ios-test.log)"
else
  echo "Tests failed. Full log: /tmp/events-ios-test.log"
fi
exit "$STATUS"
